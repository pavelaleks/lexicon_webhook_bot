...
async def continue_start(callback: CallbackQuery):
    ...
    await callback.message.answer(
        ...
        "• передать ваш вопрос Павлу Викторовичу Алексееву - просто напишите сообщение!\n\n"
...
async def about_director(callback: CallbackQuery):
    ...
    await callback.message.answer_photo(
        photo=photo,
        caption="<b>Павел Викторович Алексеев</b> — основатель и руководитель студии «Лексикон», преподаватель Горно-Алтайского университета.\n\n"
...
async def about_command(message: Message):
    await message.answer(
        "<b>Студия «Лексикон»</b>\n\n"
        "«Лексикон» — это образовательная студия для учеников 4–11 классов.\n"
        "Мы проводим занятия по школьной программе, готовим к <b>ОГЭ и ЕГЭ по русскому языку и литературе</b>, а также по истории и обществознанию, развиваем письменную и устную речь, "
        ...
        "📚 Что мы предлагаем:\n"
        "• углублённые занятия по русскому языку, литературе, истории и обществознанию\n"
...
async def location_command(message: Message):
    await message.answer("📍 Мы находимся по адресу: Горно-Алтайск, пр. Коммунистический, 47 (вход со стороны ул. Головина)", disable_web_page_preview=True)
